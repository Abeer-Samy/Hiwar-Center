...
<div class="m-portlet m-portlet--mobile">
    <form enctype="multipart/form-data" method="post" action='{{route("StudyTypeCreate")}}'>
        @csrf
        <div class='m-form'>
            <div class="m-portlet__body">
                <div class="m-form__section m-form__section--first">
                    <div class="form-group m-form__group row">
                        ...
                        <div class="col-lg-6">
                            <input type="text" name="study_type" value="{{old('study_type')}}" class="form-control m-input" placeholder="اضف نوع الدراسة">
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr role="row" class="odd">
                                ...
                                <td width="15%">
                                    <a class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only"
                                        href="{{route('StudyType.delete',$study->id)}}"
                                        onclick="return confirm('هل انت متأكد من حذف {{$study->name}} ؟')" title="حذف"><i
